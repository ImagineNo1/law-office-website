...
import { requireClientRequest } from "@/lib/client-portal-db";
...
export default async function DashboardRequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const request = await requireClientRequest(id);
    ...
    <ClientPortalShell title="جزئیات درخواست">
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]" dir="ltr">
        <div className="grid min-w-0 gap-6" dir="rtl">
          <PortalCard className="p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <p className="text-sm font-black text-gold" dir="ltr">{request.requestNumber}</p>
                <h2 className="mt-2 text-3xl font-black text-navy">{request.subject}</h2>
                <p className="mt-3 text-sm font-bold text-muted">{request.serviceTitle} · ثبت شده در {formatRequestDate(request.createdAt)}</p>
              </div>
            ...
            <p className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm font-bold leading-8 text-muted">{request.description}</p>
          </PortalCard>
          ...
          <PortalCard className="p-6">
            ...
            <form action={sendRequestMessageAction} className="mt-4 flex gap-2 rounded-2xl border border-border p-3">
              ...
              <input className="min-w-0 flex-1 bg-transparent text-sm font-bold outline-none" name="message" placeholder="ارسال پیام..." />
              <button className="rounded-xl bg-gold px-5 py-3 text-sm font-black text-white" type="submit">ارسال</button>
            </form>
          </PortalCard>

          <PortalCard className="p-6">
            <h3 className="text-xl font-black text-navy">یادداشت‌های قابل مشاهده</h3>
            {request.adminNotes.length ? (
              <div className="mt-5 grid gap-3">{request.adminNotes.slice(0, 3).map((note) => <p className="rounded-2xl bg-slate-50 p-4 text-sm font-bold leading-8 text-muted" key={note.id}>{note.message}</p>)}</div>
            ) : (
              <p className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm font-bold leading-8 text-muted">هنوز یادداشتی برای نمایش وجود ندارد.</p>
            )}
          </PortalCard>
        </div>
        <aside className="grid gap-6" dir="rtl">
          ...
          <PortalCard className="p-5"><h3 className="text-lg font-black text-navy">فایل‌ها</h3>{request.attachments.length ? <div className="mt-4 grid gap-3">{request.attachments.map((file) => <div className="flex items-center justify-between rounded-xl border border-border p-3" key={file.id}><span className="text-sm font-black text-navy">{file.filename}</span><span className="text-xs font-black text-muted">{file.size || "ثبت شده"}</span></div>)}</div> : <p className="mt-4 rounded-xl bg-slate-50 p-3 text-sm font-bold text-muted">هنوز پیوستی ثبت نشده است.</p>}</PortalCard>
        </aside>
      </div>
    </ClientPortalShell>
  );
