      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]" dir="ltr">
        <div className="grid min-w-0 gap-6" dir="rtl">
          <PortalCard className="p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <p className="mt-3 text-sm font-bold text-muted">
                  {request.serviceTitle} · ثبت شده در{" "}
            ...
            <p className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm font-bold leading-8 text-muted">
              {request.description}
            </p>
          </PortalCard>
          ...
          <PortalCard className="p-6">
            ...
            {request.messages.length ? (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="text-xs font-bold text-muted">
                        {formatRequestDate(message.createdAt)}
                      </span>
                    </div>
              ...
              <div className="mt-5">
                <EmptyState
                  description="پیام‌های مرتبط با این درخواست پس از ارسال شما یا پاسخ تیم حقوقی نمایش داده می‌شود."
                  icon="message"
                  title="هنوز پیامی برای این درخواست ثبت نشده است"
                />
              </div>
            )}
            <form
              action={sendRequestMessageAction}
              className="mt-4 flex gap-2 rounded-2xl border border-border p-3"
            >
          ...
          <PortalCard className="p-6">
            <h3 className="text-xl font-black text-navy">
              یادداشت‌های قابل مشاهده
            </h3>
            {request.adminNotes.length ? (
              <div className="mt-5 grid gap-3">
                {request.adminNotes.slice(0, 3).map((note) => (
                  <p
              ...
              <p className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm font-bold leading-8 text-muted">
                هنوز یادداشتی برای نمایش وجود ندارد.
              </p>
            )}
          </PortalCard>
        </div>
        <aside className="grid gap-6" dir="rtl">
          ...
          <PortalCard className="p-5">
            <h3 className="text-lg font-black text-navy">کارشناس مسئول</h3>
            <p className="mt-3 font-black text-muted">{request.assignedTo}</p>
            <p className="mt-2 text-sm font-bold text-muted">
              اولویت: {requestPriorityLabels[request.priority]}
            </p>
          </PortalCard>
          <PortalCard className="p-5">
            <h3 className="text-lg font-black text-navy">فایل‌ها</h3>
            {request.attachments.length ? (
              ...
              <p className="mt-4 rounded-xl bg-slate-50 p-3 text-sm font-bold text-muted">
