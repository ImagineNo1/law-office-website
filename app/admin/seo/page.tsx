...
function scoreTone(score: number) {
  ...
  if (score >= 50) return "bg-emerald-600/10 text-emerald-700";
...
function SeoScoreBadge({ score }: { score: number }) {
  return (
    <span
      className={`rounded-full px-3 py-1 text-xs font-black ${scoreTone(score)}`}
    >
...
function MetricCard({
  ...
  return (
    <div className="rounded-lg border border-border bg-white p-5 shadow-card">
      ...
      <strong className="mt-4 block text-4xl font-black text-navy">
        {value}
      </strong>
...
function Overview({
  ...
  pages: SeoPage[];
  redirects: Awaited<ReturnType<typeof getSeoRedirects>>;
}) {
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1.2fr]">
        <section className="rounded-lg border border-border bg-white p-6 shadow-card">
          ...
          <div className="mt-6 grid place-items-center">
            <div className="grid size-52 place-items-center rounded-full border-[18px] border-emerald-500/80 bg-white text-center">
              <strong className="block text-4xl font-black text-navy">
                {average}٪
              </strong>
        ...
        <section className="rounded-lg border border-border bg-white p-6 shadow-card">
          ...
          <div className="mt-4 grid gap-3 text-sm font-bold leading-8 text-muted">
            {[
              ...
              <p className="rounded-xl bg-slate-50 px-4 py-2" key={item}>
                {item}
              </p>
            ))}
          </div>
      ...
      <section className="rounded-lg border border-border bg-white shadow-card">
        <div className="border-b border-border p-5">
          <h2 className="text-xl font-black text-navy">
            آخرین صفحات نیازمند بهبود
          </h2>
        </div>
        {needsAttention.length ? (
          <PagesTable pages={needsAttention} compact />
        ) : (
          <div className="p-5">
            <AdminEmptyState
              title="همه چیز مرتب است"
              description="صفحه‌ای با امتیاز پایین پیدا نشد."
            />
          </div>
        )}
      </section>
...
function PagesTable({
  ...
  return (
    <div data-tour={compact ? "admin-seo-audit" : "admin-seo-pages"}>
      <AdminDataTable
        headers={
          compact
            ? ["صفحه", "نوع", "مسیر", "امتیاز", "عملیات"]
            : [
                "عنوان صفحه",
                "نوع صفحه",
                "مسیر",
      ...
      >
        {pages.map((page) => (
          <tr
            ...
            key={`${page.model}-${page.id}-${page.path}`}
          >
            <td className="px-5 py-4 font-black text-navy">{page.title}</td>
            <td className="px-5 py-4 font-bold text-muted">{page.typeLabel}</td>
            <td className="px-5 py-4 font-bold text-muted ltr:text-left">
              {page.path}
            </td>
            {compact ? null : (
              <td className="px-5 py-4 font-bold text-muted">
                {page.seo.robotsIndex ? "index" : "noindex"}
              </td>
            )}
            {compact ? null : (
              <td className="max-w-56 px-5 py-4 text-sm font-bold text-muted">
                {page.seo.metaTitle || "ثبت نشده"}
              </td>
            )}
            {compact ? null : (
              <td className="max-w-64 px-5 py-4 text-sm font-bold text-muted">
                {page.seo.metaDescription || "ثبت نشده"}
              </td>
            )}
            {compact ? null : (
              <td className="px-5 py-4 font-bold text-muted">
                {page.seo.focusKeyword || "ثبت نشده"}
              </td>
            )}
            ...
            <td className="px-5 py-4">
              <div className="flex flex-wrap gap-2">
                ...
                <Link
                  className="rounded-lg border border-border px-4 py-2 text-xs font-black text-navy"
                  href={page.path}
                >
                  مشاهده صفحه
                </Link>
...
function RedirectForm({
  ...
  redirect?: Awaited<ReturnType<typeof getSeoRedirects>>[number];
}) {
  return (
    <form action={saveSeoRedirectAction} className="grid gap-4">
      ...
      <label className="grid gap-2 text-sm font-black text-navy">
        ...
        <input
          className="service-input"
          defaultValue={redirect?.targetPath}
          name="targetPath"
          required
      ...
      <label className="flex items-center justify-between rounded-xl border border-border bg-white px-4 py-3 text-sm font-black text-navy">
        فعال{" "}
        <input
          defaultChecked={redirect?.enabled ?? true}
          name="enabled"
          type="checkbox"
        />
      </label>
...
function RedirectsPanel({
  ...
  return (
    <div className="grid gap-6">
      <AdminPageHeader
        title="ریدایرکت‌ها"
        description="ریدایرکت‌ها ذخیره می‌شوند؛ اعمال آن‌ها در proxy باید با احتیاط و بدون شکستن احراز هویت انجام شود."
        action={
          <AdminModal buttonLabel="افزودن ریدایرکت" title="افزودن ریدایرکت">
            <RedirectForm />
          </AdminModal>
        }
      />
      {redirects.length ? (
        <AdminDataTable
          headers={["مبدا", "مقصد", "کد", "وضعیت", "بازدید", "عملیات"]}
        >
          {redirects.map((item) => (
            <tr className="border-t border-border" key={item.id}>
              ...
              <td className="px-5 py-4">
                <div className="flex gap-2">
                  <AdminModal buttonLabel="ویرایش" title="ویرایش ریدایرکت">
        ...
        <AdminEmptyState
          ...
          description="برای آدرس‌های حذف‌شده یا تغییر مسیر داده‌شده، ریدایرکت ۳۰۱ بسازید."
        />
      )}
    </div>
  );
  ...
  return (
    <section className="rounded-lg border border-border bg-white p-5 shadow-card">
      ...
      <form action={saveSeoSettingsAction} className="mt-5 grid gap-4">
        ...
        <label className="grid gap-2 text-sm font-black text-navy">
          <span>شبکه‌های اجتماعی؛ هر خط یک مورد</span>
          <textarea
            className="service-input min-h-20 py-3"
...
export default async function AdminSeoPage({
  ...
  const [pages, redirects, settings] = await Promise.all([
    getSeoPages(),
    getSeoRedirects(),
    getSeoSettings(),
  ]);
  ...
  return (
    <AdminShell
      title="مدیریت سئو"
      description="مدیریت و بهینه‌سازی سئوی صفحات و محتوای سایت"
    >
      <div className="grid gap-6">
        <AdminPageHeader
          title="مدیریت سئو"
          description="برای هر صفحه عنوان، توضیحات، ایندکس، شبکه‌های اجتماعی، سایت‌مپ و اسکیما را از همین بخش مدیریت کنید."
        />
        <div className="rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-4 text-sm font-bold leading-8 text-muted">
          اگر با سئو آشنا نیستید، اول بخش آموزش سئو را ببینید.{" "}
          <Link
        ...
        <nav className="flex flex-wrap gap-2 rounded-lg border border-border bg-white p-2 shadow-card">
          {tabs.map(([key, label]) => (
            <Link
              className={`rounded-lg px-4 py-2 text-sm font-black ${active === key ? "bg-emerald-700 text-navy" : "text-muted hover:bg-slate-50 hover:text-navy"}`}
              href={`/admin/seo?tab=${key}`}
              key={key}
            >
              {label}
            </Link>
        ...
        {active === "overview" ? (
          <Overview pages={pages} redirects={redirects} />
        ) : null}
        {active === "pages" ? <PagesTable pages={pages} /> : null}
        {active === "audit" ? (
          <PagesTable pages={pages.filter((page) => page.score < 80)} />
        ) : null}
        {active === "redirects" ? (
        ...
        {active === "sitemap" ? (
          <PagesTable
            pages={pages.filter(
              (page) => page.seo.sitemapInclude && page.seo.robotsIndex,
            )}
            compact
          />
        ) : null}
        ...
        {active === "learn" ? (
          <section className="rounded-lg border border-border bg-white p-6 shadow-card">
            ...
            <div className="mt-4 grid gap-3 text-sm font-bold leading-8 text-muted">
              ...
              <p>
                توضیحات متا باید به کاربر بگوید صفحه درباره چیست و چرا باید کلیک
                کند.
              </p>
              <p>
                صفحات خصوصی، داشبورد، لاگین و پنل مدیریت همیشه باید از سایت‌مپ
                خارج باشند.
              </p>
              <p>
                برای محتوای حذف‌شده یا تغییر آدرس داده‌شده، ریدایرکت ۳۰۱ بسازید.
              </p>
            </div>
          </section>
        ) : null}
