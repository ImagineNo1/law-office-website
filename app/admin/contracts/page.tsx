...
const faqLines = (items?: { question: string; answer: string }[]) =>
  ...
  return (
    <form action={saveContractAction} className="grid gap-4">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <SlugField defaultValue={contract?.slug} />
        <CategoryField
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        />
        <UploadField
          defaultValue={contract?.sampleFileUrl}
          fileName="sampleFileUrlFile"
          label="تصویر یا فایل نمونه"
          name="sampleFileUrl"
        />
        <label className="grid gap-2 text-sm font-black text-navy">
          <span>وضعیت</span>
      ...
      <Area
        defaultValue={contract?.excerpt}
        label="خلاصه"
        name="excerpt"
        required
      />
      <Area defaultValue={contract?.content} label="محتوا" name="content" />
      <Area
        defaultValue={lines(contract?.benefits)}
        label="مزایا؛ هر خط یک مورد"
        name="benefits"
      />
      <Area
        defaultValue={lines(contract?.useCases)}
        label="موارد کاربرد؛ هر خط یک مورد"
        name="useCases"
      />
      <Area
        defaultValue={lines(contract?.requiredDocuments)}
        label="مدارک مورد نیاز؛ هر خط یک مورد"
        name="requiredDocuments"
      />
      <Area
        defaultValue={faqLines(contract?.faqItems)}
        label="سوالات متداول؛ سوال | پاسخ"
        name="faqItems"
      />
      <SeoFields seo={contract?.seo} title={String(contract?.title ?? "")} />
      <AdminSubmitButton />
    </form>
  );
}
...
export default async function AdminContractsPage() {
  ...
  const categories = contracts.map((contract) => contract.category ?? "ملکی");
  return (
    <AdminShell title="قراردادها" description="مدیریت بانک قراردادها">
      <div className="grid gap-6">
        <AdminPageHeader
          title="قراردادها"
          description="مدیریت قراردادهای ثبت‌شده و قابل انتشار در سایت."
          action={
            <AdminModal buttonLabel="افزودن قرارداد" title="افزودن قرارداد">
              <ContractForm categories={categories} />
            </AdminModal>
          }
        />
        ...
        {contracts.length ? (
          ...
          >
            {contracts.map((contract) => (
              <tr className="border-t border-border" key={contract.id}>
                ...
                <td className="px-5 py-4 font-bold text-muted">
                  {contract.priceLabel || "ثبت نشده"}
                </td>
                <td className="px-5 py-4 font-bold text-muted">
                  {contract.updatedAtText}
                </td>
                <td className="px-5 py-4">
                  <div className="flex flex-wrap gap-2">
                    <AdminModal
                      buttonLabel="ویرایش"
                      title={`ویرایش ${contract.title}`}
                    >
                      <ContractForm
                        categories={categories}
                        contract={contract}
          ...
          <AdminEmptyState
            ...
            description="اولین قرارداد را از دکمه افزودن قرارداد ایجاد کنید."
          />
        )}
      </div>
    </AdminShell>
