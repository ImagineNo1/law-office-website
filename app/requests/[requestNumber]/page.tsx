...
export default async function RequestTrackingPage({
  ...
  const request = await getServiceRequestById(decodeURIComponent(requestNumber));
  if (!request) notFound();
      ...
      <section className="py-8">
        <div className="mx-auto grid w-[min(1200px,calc(100%-32px))] gap-6 lg:grid-cols-[1fr_340px]">
          <div className="rounded-2xl bg-white p-6 shadow-[0_18px_45px_rgba(11,23,42,.06)]">
            <h1 className="text-3xl font-black">پیگیری درخواست {request.requestNumber}</h1>
            <p className="mt-3 text-sm font-bold text-[#66758A]">{request.serviceTitle} برای {request.fullName}</p>
