...
function Icon({ name }: { name: "login" | "signup" | "mail" | "lock" }) {
  const paths = {
    ...
    signup:
      "M16 21v-2a4 4 0 0 0-8 0v2M12 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm7-5v6M22 9h-6",
  ...
  return (
    ...
    >
      <section className="w-full max-w-md rounded-2xl border border-border/50 bg-card p-8 shadow-xl shadow-primary/5">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm font-bold text-muted-foreground">
            {isLogin
              ...
              : "برای استفاده از سامانه ثبت‌نام کنید"}
          </p>
        </div>
        ...
        <form
          action={isLogin ? loginClientAction : signupClientAction}
          className="space-y-4"
        >
          ...
          <button
            className="h-12 w-full rounded-lg bg-primary text-sm font-black text-primary-foreground transition hover:bg-primary/90"
            type="submit"
          >
            {isLogin ? "ورود به حساب" : "ساخت حساب کاربری"}
          </button>
        ...
        <p className="mt-6 text-center text-sm font-bold text-muted-foreground">
          ...
          <Link
            ...
            href={`${isLogin ? "/signup" : "/login"}?next=${encodeURIComponent(next)}`}
          >
