...
function FileIcon() {
  return (
    <svg aria-hidden="true" className="size-5" viewBox="0 0 24 24" fill="none">
      <path
        d="M7 3h7l5 5v13H7V3Zm7 0v6h6M10 13h6M10 17h4"
        stroke="currentColor"
        strokeLinecap="round"
...
function ContractImage({
  ...
  const fallbackImages = [
    "/legal-scene-light-final.png",
    "/home-legal-desk-hero.png",
    "/legal-scene-dark-final.png",
  ];
  const image =
    contract.heroImage || fallbackImages[index % fallbackImages.length];
        ...
        <div className="grid gap-8 lg:grid-cols-[0.78fr_1.22fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 text-sm font-bold leading-8 text-[#64748B]">
              دسترسی به صدها قرارداد استاندارد و تخصصی متناسب با پرونده یا
              کسب‌وکار شما.
            </p>
            <Link
              className="mt-8 inline-flex items-center gap-2 rounded-2xl border border-[#0F766E]/30 bg-white px-5 py-3 text-sm font-black text-[#0F766E] transition hover:bg-[#ECFDF5]"
              href="/contracts"
            >
            ...
            {items.slice(0, 3).map((contract, index) => (
              ...
              >
                <div className="h-36 overflow-hidden bg-[#F8FAFC]">
                  <ContractImage contract={contract} index={index} />
                </div>
                <div className="p-4">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <span className="rounded-full bg-[#ECFDF5] px-3 py-1 text-[11px] font-black text-[#0F766E]">
                      {contract.category || "قرارداد"}
                    </span>
                    <span className="grid size-9 place-items-center rounded-xl bg-[#F8FAFC] text-[#0F766E]">
                      <FileIcon />
                    </span>
                  </div>
                  <h3 className="line-clamp-2 min-h-14 text-sm font-black leading-7 text-[#10233B]">
                    {contract.title}
                  </h3>
                  <p className="mt-2 line-clamp-2 text-xs font-bold leading-6 text-[#64748B]">
                    {contract.description ||
                      "قالب حقوقی آماده با بندهای ضروری، چک‌لیست مدارک و امکان سفارشی‌سازی."}
                  </p>
