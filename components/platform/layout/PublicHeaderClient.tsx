...
export function PublicHeaderClient({
  ...
  return (
    <header
      className="sticky inset-x-0 top-0 z-50 border-b border-[#E2E8F0] bg-white/95 font-body shadow-[0_12px_35px_rgba(7,21,39,0.04)] backdrop-blur"
      dir="rtl"
    >
        ...
        <div className="mx-auto flex min-h-9 max-w-[1540px] items-center justify-center gap-5 px-5 text-[11px] font-black text-[#10233B] sm:px-8 lg:justify-between lg:px-16">
          {trustItems.map((item) => (
            <span
              ...
              key={item}
            >
              <span className="grid size-5 place-items-center rounded-full bg-[#ECFDF5] text-[#0F766E]">
                <CheckIcon />
              </span>
              {item}
            </span>
          ))}
          <span className="inline-flex items-center gap-2 text-[#0F766E] md:hidden">
            <CheckIcon /> پشتیبانی آنلاین و پیگیری سریع
          </span>
        </div>
      </div>
      ...
      <nav className="mx-auto flex min-h-20 w-full max-w-[1540px] items-center justify-between gap-5 px-5 sm:px-8 lg:px-16">
        ...
        <div className="hidden min-w-72 items-center justify-end gap-3 font-body md:flex">
          <Link
            className="inline-flex h-12 items-center gap-2 rounded-2xl bg-[#071527] px-5 text-sm font-black text-white shadow-[0_14px_30px_rgba(7,21,39,0.18)] transition hover:bg-[#10233B]"
            href={dashboardHref}
          >
          ...
          <Link
            className="inline-flex h-12 items-center gap-2 rounded-2xl bg-[#0F766E] px-5 text-sm font-black text-white shadow-[0_14px_30px_rgba(15,118,110,0.18)] transition hover:bg-[#0b625c]"
        ...
        <button
          className="rounded-2xl border border-[#E2E8F0] p-3 text-[#071527] xl:hidden"
      ...
      {mobileOpen ? (
        <div className="border-t border-[#E2E8F0] bg-white xl:hidden">
          <div className="space-y-3 px-5 py-4 sm:px-8">
            {links.map((link) => (
              <Link
                className="block rounded-2xl px-3 py-2 text-sm font-black text-[#10233B] hover:bg-[#F8FAFC]"
                href={link.href}
                key={link.label}
            ...
            <div className="grid gap-3 pt-2 sm:grid-cols-2">
              <Link
                className="flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-[#071527] px-4 text-sm font-black text-white"
                href={dashboardHref}
              >
                {isLoggedIn ? "داشبورد" : "ورود"}
              </Link>
              <Link
                className="flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-[#0F766E] px-4 text-sm font-black text-white"
                href="/dashboard/requests?new=1"
              >
                ثبت درخواست جدید
              </Link>
