        ...
        <div>
          ...
          <div className="grid gap-2">
            {fallbackServices.slice(0, 5).map((service) => (
              <Link className="rounded-2xl p-3 transition hover:bg-slate-50" href={`/services/${service.slug}`} key={service.slug}>
        ...
        <div>
          ...
          <div className="grid gap-2">
            {fallbackContracts.slice(0, 5).map((contract) => (
              <Link className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-3 py-3 text-sm font-black transition hover:bg-slate-50" href={`/contracts/${contract.category}/${contract.slug}`} key={contract.id}>
                <span>{contract.title}</span>
                <span className="rounded-full bg-[#FFF8EA] px-2 py-1 text-xs text-[#C9973F]">{contract.category}</span>
              </Link>
            ))}
          </div>
        </div>
        <div className="rounded-2xl bg-[#0B172A] p-4 text-white">
          ...
          <div className="mt-3 grid gap-2">
            {fallbackLegalForms.slice(0, 4).map((form) => (
              <Link className="rounded-xl bg-white/8 px-3 py-3 text-sm font-black transition hover:bg-white/12" href="/legal-forms" key={form.id}>
                {form.title}
              </Link>
            ))}
          </div>
          <Link className="mt-4 flex h-11 items-center justify-center rounded-xl bg-white text-sm font-black text-[#0B172A]" href="/requests/new">
            شروع درخواست
          </Link>
        </div>
      </div>
