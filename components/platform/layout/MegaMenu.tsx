...
import { recoveryServices, recoveryContracts, legalForms } from "@/lib/platform-recovery-data";
      ...
      <div className="grid grid-cols-[1.1fr_0.9fr_0.8fr] gap-4 rounded-[26px] border border-[#eadfce] bg-white p-5 text-[#0B172A] shadow-[0_30px_90px_rgba(11,23,42,.18)]">
        <div>
          ...
          <div className="grid gap-2">
            {recoveryServices.slice(0, 5).map(([title, slug, description]) => (
              <Link className="rounded-2xl p-3 hover:bg-[#F7F3EA]" href={`/services/${slug}`} key={slug}>
        ...
        <div>
          ...
          <div className="grid gap-2">
            {recoveryContracts.slice(0, 5).map((contract) => (
