...
export function ContractCard({ contract }: { contract: PlatformContract }) {
  return (
    <Link className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_45px_rgba(11,23,42,.06)] transition hover:-translate-y-1 hover:border-[#C9973F]" href={`/contracts/${contract.category}/${contract.slug}`}>
      <div className="flex items-center justify-between">
        <IconBox>□</IconBox>
        <span className="rounded-full bg-slate-50 px-3 py-1 text-xs font-black">{contract.category}</span>
      </div>
