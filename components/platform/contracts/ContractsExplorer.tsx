...
export function ContractsExplorer({
  ...
  const categories = useMemo(
    () => [
      "همه قراردادها",
      ...Array.from(
        new Set(contracts.map((item) => item.category).filter(Boolean)),
      ),
    ],
    [contracts],
  );
  ...
  const filtered = contracts
    .filter((contract) => {
      const matchesCategory =
        category === "همه قراردادها" || contract.category === category;
      const searchable =
        `${contract.title} ${contract.category} ${contract.description}`.toLowerCase();
      return (
        matchesCategory &&
      ...
      <aside className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_45px_rgba(11,23,42,.06)] lg:sticky lg:top-28 lg:self-start">
          ...
          {categories.map((item) => (
            ...
            >
              ...
              <span>
                {item === "همه قراردادها"
                  ? contracts.length
                  : contracts.filter((contract) => contract.category === item)
                      .length}
              </span>
            </button>
          ))}
        ...
        <div className="mt-5 rounded-2xl bg-[#0B172A] p-4 text-white">
          ...
          <p className="mt-2 text-xs font-bold leading-6 text-slate-300">
            اگر قالب آماده کافی نیست، تنظیم اختصاصی را ثبت کنید.
          </p>
        </div>
      </aside>
      <div>
        <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_18px_45px_rgba(11,23,42,.05)] md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <select
              className="h-11 rounded-xl border border-slate-200 px-4 text-sm font-bold"
        ...
        {filtered.length ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-[0_18px_45px_rgba(11,23,42,.04)]">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm font-bold leading-7 text-slate-500">
              جستجو یا دسته‌بندی را تغییر دهید، یا درخواست قرارداد اختصاصی ثبت
              کنید.
            </p>
          </div>
        )}
