    ...
    <div className="overflow-hidden rounded-2xl bg-white shadow-[0_18px_45px_rgba(11,23,42,.06)]">
      <div className="flex flex-col gap-3 border-b border-slate-200 p-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex gap-2">
          <input className="h-11 rounded-xl border border-slate-200 px-4 text-sm font-bold" placeholder="جستجو..." />
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[900px] text-sm">
          <thead className="bg-slate-50 text-[#66758A]">
          ...
          <tbody>
            {requests.slice(0, 14).map((request) => (
              <tr className="border-t border-slate-200" key={request.id}>
                ...
                <td className="px-5 py-4"><Link className="rounded-lg border border-slate-200 px-3 py-2 font-black" href={`/admin/requests/${request.id}`}>مشاهده</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
