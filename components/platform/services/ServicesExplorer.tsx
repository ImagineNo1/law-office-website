...
export function ServicesExplorer({ services }: { services: PlatformService[] }) {
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState("همه خدمات");
  const categories = useMemo(() => ["همه خدمات", ...Array.from(new Set(services.map((item) => item.category || item.tag).filter(Boolean)))], [services]);
  const normalizedQuery = query.trim().toLowerCase();
  const filtered = services.filter((service) => {
    const matchesCategory = category === "همه خدمات" || service.category === category || service.tag === category;
    const searchable = `${service.title} ${service.description} ${service.category} ${service.tag}`.toLowerCase();
      ...
      <div>
        {filtered.length ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {filtered.map((item) => (
              <ServiceCard desc={item.description} key={item.slug} sla={item.sla} slug={item.slug} tag={item.tag} title={item.title} />
            ))}
          </div>
        ) : (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-[0_18px_45px_rgba(11,23,42,.04)]">
            <h2 className="text-2xl font-black text-[#0B172A]">خدمتی پیدا نشد</h2>
            <p className="mx-auto mt-3 max-w-md text-sm font-bold leading-7 text-slate-500">عبارت جستجو یا دسته‌بندی را تغییر دهید، یا درخواست حقوقی جدید ثبت کنید.</p>
          </div>
        )}
      </div>
    </>
