  ...
  return (
    <aside className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_45px_rgba(11,23,42,.06)] lg:sticky lg:top-28 lg:self-start">
      ...
      <div className="mt-4 grid gap-2">
        {["همه خدمات", "قرارداد", "دعاوی", "کیفری", "امضا", "پرونده"].map(
          (item, index) => (
            <button
              className={`rounded-xl px-4 py-3 text-right text-sm font-black ${index === 0 ? "bg-[#0B172A] text-white" : "bg-slate-50 text-[#0B172A]"}`}
              key={item}
            >
              {item}
            </button>
