...
import { legalForms, fa } from "@/lib/platform-recovery-data";
      ...
      <div className="mx-auto w-[min(1440px,calc(100%-32px))]">
        <div className="rounded-2xl bg-[#0B172A] p-7 text-white">
          ...
          <p className="mt-3 text-sm font-bold text-slate-300">
            ۸ فرم route-backed برای تکمیل، امضا، آرشیو و مدیریت در سامانه.
          </p>
        </div>
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {legalForms.map((form) => (
            <div className="rounded-2xl border border-[#eadfce] bg-white p-5 shadow-[0_18px_45px_rgba(11,23,42,.06)]" key={form.id}>
              <span className="grid size-11 place-items-center rounded-2xl bg-[#F4E9D3] text-sm font-black text-[#C9973F]">F</span>
