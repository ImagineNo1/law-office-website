    ...
    <div>
      ...
      <div className="mt-5 grid gap-3">
        {links.map(([label, href]) => (
          <Link
            className="text-sm font-bold text-white/68 transition hover:text-emerald-700"
            href={href}
            key={label}
            prefetch={false}
          >
...
export function SiteFooter({ settings }: { settings: SiteSettings }) {
  ...
  const title =
    settings.siteTitle &&
    !/[ØÙÛ]/.test(settings.siteTitle) &&
    settings.siteTitle.includes("موسسه")
      ? settings.siteTitle
    ...
    <footer className="bg-[#071225] text-white">
      <Container className="grid gap-10 border-t border-white/10 py-16 lg:grid-cols-[1.35fr_0.75fr_0.75fr_0.9fr_1fr]">
        <div>
          ...
          <div className="mt-6 h-px w-28 bg-emerald-700" />
        </div>
      ...
      <div className="border-t border-white/10 py-5">
        <Container className="flex flex-col items-center justify-between gap-3 text-xs font-bold text-white/48 sm:flex-row">
          <span>تمام حقوق برای {title} محفوظ است.</span>
          <span className="text-emerald-700">
            LegalTech Foundation, Phase 1
          </span>
        </Container>
      </div>
    </footer>
