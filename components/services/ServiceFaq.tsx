...
export function ServiceFaq({ items }: { items: NonNullable<Service["faqItems"]> }) {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <section className="rounded-[8px] border border-border bg-white p-5 shadow-card">
      <h2 className="mb-4 text-center text-xl font-black text-navy">سوالات متداول</h2>
      <div className="grid gap-2">
        {items.map((item, index) => {
          const open = index === openIndex;

          return (
            <div className="overflow-hidden rounded-xl border border-border bg-white" key={item.question}>
              <button
                aria-expanded={open}
