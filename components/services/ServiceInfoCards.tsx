    ...
    <div className="grid gap-3 md:grid-cols-3">
      <article className="rounded-[8px] border border-border bg-white p-6 shadow-card">
        <div className="mb-5 flex items-center justify-between">
          ...
          <span className="grid size-14 place-items-center rounded-full bg-[#FBF4E8] text-emerald-700">
        ...
        <ul className="grid gap-3">
          {needItems.map((item) => (
            <li
              className="flex items-center gap-2 text-sm font-bold leading-7 text-muted"
              key={item}
            >
      ...
      <article className="rounded-[8px] border border-border bg-white p-6 shadow-card">
        ...
        <ol className="grid gap-3">
          {(service.processSteps ?? []).slice(0, 5).map((step, index) => (
            <li
              className="grid grid-cols-[30px_1fr] items-center gap-3 text-sm font-bold leading-7 text-muted"
              key={step}
            >
              <span className="grid size-7 place-items-center rounded-full bg-emerald-700/15 text-xs font-black text-emerald-700">
