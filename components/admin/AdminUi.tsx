...
export function AdminStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full px-3 py-1 text-xs font-extrabold ${tone}`}
    >
      {adminStatusLabels[status] ?? status}
    </span>
  );
}
...
export function AdminEmptyState({
  ...
  return (
    <div className="rounded-lg border border-dashed border-border bg-white p-8 text-center">
      ...
      <p className="mx-auto mt-2 max-w-md text-sm font-medium leading-8 text-muted-foreground">
        {description}
      </p>
...
export function AdminActionLink({
  ...
  return (
    <Link
      className="rounded-lg border border-border px-3 py-2 text-xs font-extrabold text-primary transition hover:border-accent hover:text-accent"
      href={href}
    >
      {children}
    </Link>
