...
const statusClass: Record<RequestStatus, string> = {
  ...
  reviewing: "bg-[#ECFDF5] text-emerald-700 ring-emerald-200",
  waiting_for_client: "bg-[#ECFDF5] text-orange-700 ring-orange-200",
...
export function StatusBadge({ status }: { status: RequestStatus }) {
  return (
    <span
      className={`inline-flex rounded-full px-3 py-1 text-xs font-black ring-1 ${statusClass[status]}`}
    >
...
export function PriorityBadge({ priority }: { priority: RequestPriority }) {
  return (
    <span
      className={`inline-flex rounded-full px-3 py-1 text-xs font-black ${priorityClass[priority]}`}
    >
...
export function MiniLineChart({
  ...
  const points = data
    .map(
      (item, index) =>
        `${(index / Math.max(data.length - 1, 1)) * 100},${100 - (item.value / max) * 78 - 10}`,
    )
    .join(" ");
  ...
  return (
    <div className="h-56 rounded-2xl border border-border bg-white p-4">
      ...
      >
        <defs>
          <linearGradient id="greenLine" x1="0" x2="1">
    ...
    <div className="grid items-center gap-4 rounded-2xl border border-border bg-white p-4 sm:grid-cols-[180px_1fr]">
      <svg viewBox="0 0 42 42" className="mx-auto size-40 rotate-[-90deg]">
        ...
        <text
          ...
          rotate="90 21 21"
          textAnchor="middle"
          className="fill-navy text-[5px] font-black"
        >
          {total}
        </text>
        <text
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-white shadow-card">
      <div className="flex flex-col gap-3 border-b border-border p-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="grid gap-2 sm:grid-cols-5">
          <input
            className="service-input sm:col-span-2"
            placeholder="جستجو در درخواست‌ها..."
          />
          <select className="service-input">
        ...
        <table className="w-full min-w-[980px] text-sm">
          <thead className="bg-slate-50 text-xs text-muted">
            <tr>
              {[
                ...
                <th className="px-5 py-4 text-right font-black" key={heading}>
                  {heading}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {requests.slice(0, 10).map((request) => (
              <tr
                className="border-t border-border/70 hover:bg-slate-50"
                key={request.id}
              >
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2">
                    <Link
                      className="rounded-lg border border-border px-3 py-2 font-black text-navy hover:border-emerald-500 hover:text-emerald-700"
                      href={`/admin/requests/${request.id}`}
                    >
                      مشاهده
                    </Link>
                    <button className="rounded-lg border border-border px-3 py-2 font-black text-muted">
                      ویرایش
                    </button>
                    <button className="rounded-lg border border-border px-3 py-2 font-black text-muted">
                      ارجاع
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border p-4 text-sm font-bold text-muted">
        ...
        <div className="flex gap-2">
          ...
          <button className="rounded-lg bg-emerald-700 px-3 py-2 text-white">
            ۱
          </button>
          <button className="rounded-lg border border-border px-3 py-2">
            بعدی
