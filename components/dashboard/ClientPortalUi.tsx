...
const statusColors: Record<string, string> = {
  ...
  in_progress: "bg-emerald-600/10 text-emerald-700 ring-1 ring-emerald-500/15",
  waiting_for_client:
    "bg-emerald-600/10 text-emerald-700 ring-1 ring-emerald-500/15",
  ...
  draft: "bg-emerald-600/10 text-emerald-700 ring-1 ring-emerald-500/15",
  ...
  pending: "bg-emerald-600/10 text-emerald-700 ring-1 ring-emerald-500/15",
...
export function PortalCard({
  ...
  return (
    <section className={`panel-card rounded-lg ${className}`} {...props}>
      {children}
    </section>
  );
}
...
export function EmptyState({
  ...
  return (
    <div
      className="grid place-items-center rounded-lg border border-dashed border-border bg-white px-6 py-12 text-center"
      data-tour="client-empty-state"
    >
      <span className="grid size-14 place-items-center rounded-lg bg-card text-accent shadow-sm">
        <Icon aria-hidden="true" className="size-6" />
      </span>
      <h3 className="mt-5 font-heading text-lg font-extrabold text-primary">
        {title}
      </h3>
      <p className="mt-2 max-w-md text-sm font-medium leading-8 text-muted-foreground">
        {description}
      </p>
      {ctaHref && ctaLabel ? (
        <Link
          className="mt-5 rounded-lg bg-accent px-5 py-3 text-sm font-extrabold text-white transition hover:bg-accent/90"
          href={ctaHref}
        >
          {ctaLabel}
        </Link>
  ...
  return (
    <PortalCard className="grid min-h-56 place-items-center border-dashed border-slate-300 p-6 text-center shadow-sm">
      <div>
        ...
        <p className="mx-auto mt-2 max-w-sm text-sm font-medium leading-8 text-muted-foreground">
          {description}
        </p>
        {href && label ? (
          <Link
            className="mt-5 inline-flex rounded-lg border border-border bg-white px-5 py-3 text-sm font-extrabold text-primary transition hover:border-accent hover:text-accent"
            href={href}
          >
            {label}
          </Link>
...
export function ClientKpiCards({
  ...
  kpis: { label: string; value: number; hint: string; icon: string }[];
}) {
          ...
          <PortalCard className="p-5" key={kpi.label}>
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <strong className="mt-4 block text-4xl font-extrabold tabular-nums text-primary">
                  {new Intl.NumberFormat("fa-IR").format(kpi.value)}
                </strong>
                <span className="mt-2 inline-flex items-center gap-2 text-xs font-bold text-muted-foreground">
                  <span className="size-2 rounded-full bg-emerald-500" />
                  {kpi.hint}
                </span>
              </div>
              <span
                className={`grid size-14 place-items-center rounded-lg ${colors[index % colors.length]}`}
              >
                <Icon aria-hidden="true" className="size-6" />
              </span>
            </div>
...
export function ProfileCard({ profile }: { profile: ClientProfileData }) {
  return (
    <PortalCard className="p-6 text-center">
      <h2 className="mb-5 text-right text-lg font-black text-navy">
        اطلاعات کاربری
      </h2>
      ...
      <Link
        className="mt-5 inline-flex w-full justify-center rounded-xl border border-border px-5 py-3 text-sm font-black text-navy transition hover:border-emerald-500 hover:text-emerald-700"
...
export function RequestStatusDonut({
  ...
  data: { status: RequestStatus; value: number }[];
}) {
  ...
  return (
    <PortalCard className="p-6">
      <h2 className="mb-5 text-lg font-black text-navy">
      ...
      <div className="grid items-center gap-6 lg:grid-cols-[1fr_220px]">
          ...
          <div className="grid gap-4">
            {data.map((item) => (
              ...
              >
                ...
                <strong className="text-sm tabular-nums text-navy">
                  {item.value}
                </strong>
              </div>
            ))}
        ...
        <svg viewBox="0 0 42 42" className="mx-auto size-56 rotate-[-90deg]">
          ...
          <text
            ...
            rotate="90 21 21"
            textAnchor="middle"
            className="fill-navy text-[6px] font-black"
          >
            {total}
          </text>
          <text
    ...
    <PortalCard className="overflow-hidden" data-tour="client-recent-requests">
      <div className="flex flex-col gap-3 border-b border-border p-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <form className="flex flex-col gap-2 sm:flex-row">
          ...
          <button
            className="rounded-xl bg-navy px-4 py-2 text-sm font-black text-white"
            type="submit"
          >
            اعمال
          </button>
      ...
      {requests.length ? (
            ...
            <thead className="bg-slate-50 text-muted">
              <tr>
                {[
                  ...
                  <th className="px-5 py-4 text-right font-black" key={item}>
                    {item}
                  </th>
                ))}
              </tr>
              ...
              {requests.map((request) => (
                ...
                >
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-black ${statusColors[request.status]}`}
                    >
        ...
        <div className="p-5">
          <div className="grid gap-4">
            <EmptyState
              description="برای شروع، درخواست حقوقی جدیدی ثبت کنید تا روند بررسی و پیگیری آن در همین بخش نمایش داده شود."
              icon="request"
...
export function ContractsList({
  ...
  if (!contracts.length) {
    return (
      <EmptyState
        ctaHref="/contracts"
        ctaLabel="مشاهده بانک قراردادها"
        description="پس از خرید یا تخصیص قرارداد، نسخه‌های آماده دانلود شما در این بخش قرار می‌گیرد."
        icon="contract"
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {contracts.map((contract) => (
        <PortalCard className="p-5" key={contract.id}>
          <div className="flex items-start justify-between gap-3">
            <span className="grid size-12 place-items-center rounded-lg bg-blue-500/10 text-blue-700">
              <FileText aria-hidden="true" className="size-6" />
            </span>
            <span
              className={`rounded-full px-3 py-1 text-xs font-extrabold ${statusColors[contract.status]}`}
            >
          ...
          <p className="mt-2 text-sm font-medium text-muted-foreground">
            {contract.category} · تاریخ خرید: {contract.purchaseDate}
          </p>
          {contract.fileUrl ? (
            <a
              className="mt-5 inline-flex items-center gap-2 rounded-lg border border-border px-5 py-3 text-sm font-extrabold text-primary hover:border-accent hover:text-accent"
            ...
            <span className="mt-5 inline-flex rounded-lg border border-border bg-white px-5 py-3 text-sm font-extrabold text-muted-foreground">
              فایل هنوز آماده نیست
            </span>
          )}
        </PortalCard>
      ))}
  ...
  return (
    <PortalCard className="overflow-hidden">
      ...
      {files.length ? (
        <div className="overflow-x-auto">
          <table className="dashboard-table w-full min-w-[720px] text-sm">
            <thead className="bg-white text-muted-foreground">
              <tr>
                {["نام فایل", "نوع", "تاریخ آپلود", "حجم", "عملیات"].map(
                  (item) => (
                    <th
            ...
            <tbody>
              {files.map((file) => (
                <tr className="border-t border-border" key={file.id}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex gap-2">
                      {file.previewUrl ? (
                        <a
                          className="inline-flex items-center gap-1 rounded-lg border border-border px-3 py-2 text-xs font-extrabold"
                        ...
                        <span className="rounded-lg border border-border px-3 py-2 text-xs font-extrabold text-muted-foreground">
                          پیش‌نمایش ندارد
                        </span>
                      )}
                      {file.fileUrl ? (
                        <a
                          className="inline-flex items-center gap-1 rounded-lg bg-accent px-3 py-2 text-xs font-extrabold text-white"
                          href={file.fileUrl}
                        >
                          <Download aria-hidden="true" className="size-4" />
                          دانلود
                        ...
                        <span className="rounded-lg border border-border bg-white px-3 py-2 text-xs font-extrabold text-muted-foreground">
                          دانلود ندارد
                        </span>
                      )}
                    </div>
        ...
        <div className="p-5">
          <EmptyState
            description="هر فایلی که توسط شما یا تیم حقوقی برای پرونده‌ها ثبت شود اینجا قابل مشاهده خواهد بود."
            icon="file"
      ...
      {payments.length ? (
        <div className="overflow-x-auto">
          <table className="dashboard-table w-full min-w-[720px] text-sm">
            <thead className="bg-white text-muted-foreground">
              <tr>
                {["شماره فاکتور", "تاریخ", "مبلغ", "وضعیت", "عملیات"].map(
                  (item) => (
                    <th
                      className="px-5 py-4 text-right font-extrabold"
                      key={item}
            ...
            <tbody>
              {payments.map((payment) => (
                <tr className="border-t border-border" key={payment.id}>
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-extrabold ${statusColors[payment.status]}`}
                    >
                      {paymentStatusLabels[payment.status]}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    <button className="rounded-lg border border-border px-3 py-2 text-xs font-extrabold">
                      مشاهده
                    </button>
                  </td>
                </tr>
        ...
        <div className="p-5">
          <EmptyState
            description="فاکتورها و پرداخت‌های ثبت‌شده شما پس از صدور در این بخش نمایش داده می‌شوند."
            icon="payment"
    ...
    <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
      <PortalCard className="p-5">
        ...
        <form
          ...
          className="mb-4 grid gap-3 rounded-lg border border-border bg-white p-3"
        >
          <input
            className="service-input"
            name="threadTitle"
            placeholder="عنوان چت جدید"
            required
          />
          <input
            className="service-input"
            name="message"
            placeholder="اولین پیام..."
            required
          />
          <button
        ...
        {conversations.length ? (
          <div className="grid gap-2">
            {conversations.map((conversation, index) => (
              <div
                className={`rounded-lg border p-4 ${index === 0 ? "border-accent bg-accent/5" : "border-border bg-card"}`}
          ...
          <EmptyState
            description="برای شروع، یک چت جدید با تیم حقوقی ایجاد کنید."
            icon="message"
      ...
      <PortalCard className="p-5">
        <div className="mb-5 flex items-center justify-between">
          <h2 className="font-heading text-xl font-extrabold text-primary">
            {activeConversation?.title ?? "چت جدید"}
          </h2>
          <span className="rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-extrabold text-emerald-700">
            پاسخگویی فعال
          </span>
        </div>
        {activeConversation ? (
          ...
          <EmptyState
            description="از ستون گفتگوها یک چت جدید بسازید تا پیام‌ها اینجا نمایش داده شوند."
            icon="message"
        ...
        >
          <input
            name="threadId"
            type="hidden"
            value={activeConversation?.threadId ?? "general"}
          />
          <input
            name="threadTitle"
            type="hidden"
          ...
          />
          <button
            className="inline-flex items-center gap-2 rounded-lg bg-accent px-5 py-3 text-sm font-extrabold text-white"
            type="submit"
          >
