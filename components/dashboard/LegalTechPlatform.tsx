...
const statusClass: Record<string, string> = {
  ...
  waiting_signature: "bg-emerald-50 text-emerald-700",
  ...
  paused: "bg-emerald-50 text-emerald-700",
...
const iconPaths: Record<string, string> = {
  ...
  contacts:
    "M16 21v-2a4 4 0 0 0-8 0v2M12 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm7 10v-2.5a3.5 3.5 0 0 0-2.3-3.3",
  send: "M21 3 10 14M21 3l-7 18-4-7-7-4 18-7Z",
  workflow:
    "M6 6h.01M18 6h.01M6 18h.01M18 18h.01M6 6h12M6 6v12M18 6v12M6 18h12",
...
function Card({
  ...
  return (
    <section
      className={`rounded-xl border border-border/50 bg-card ${className}`}
    >
      {children}
    </section>
  );
...
function EmptyState({
  ...
  return (
    <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
      <span className="mx-auto grid size-12 place-items-center rounded-2xl bg-white text-[#0F766E] shadow-sm">
        <Icon name="files" />
      </span>
...
function Badge({ status }: { status: string }) {
  return (
    <span
      className={`inline-flex rounded-full px-3 py-1 text-xs font-black ${statusClass[status] ?? "bg-slate-100 text-slate-700"}`}
    >
          ...
          <div className="flex h-16 items-center justify-between px-4 sm:px-6">
            <div className="hidden items-center gap-2 rounded-lg bg-muted/50 px-3 py-2 sm:flex sm:w-80">
              ...
              <input
                ...
                placeholder="جستجو در درخواست‌ها، قراردادها، فایل‌ها..."
              />
            </div>
...
function KpiGrid({ data }: { data: DashboardData }) {
  const cards = [
    ...
    },
    {
      ...
      value: data.stats.pendingSignatures,
      icon: "pen",
      accent: "text-emerald-700 bg-emerald-50",
    },
    {
      label: "امضا شده",
      value: data.stats.signedDocuments,
      icon: "workflow",
      accent: "text-emerald-700 bg-emerald-50",
    },
    {
      ...
      {cards.map((card) => (
        ...
        >
          <div className="mb-3 flex items-start justify-between">
            <span
              className={`grid size-10 place-items-center rounded-xl ${card.accent}`}
            >
              <Icon name={card.icon} />
            </span>
          ...
          <strong className="block text-2xl font-extrabold text-foreground">
            {toFaNumber(card.value)}
          </strong>
      ...
      {documents.length ? (
                ...
                <tr className="transition hover:bg-slate-50" key={document.id}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2 text-xs font-black text-slate-500">
                      <button className="rounded-xl border border-slate-200 px-3 py-2 transition hover:border-[#0F766E] hover:text-[#0F766E]">
                        نمایش
                      </button>
                      <button className="rounded-xl border border-slate-200 px-3 py-2 transition hover:border-[#0F766E] hover:text-[#0F766E]">
                        دانلود
                      </button>
                      <button className="rounded-xl border border-slate-200 px-3 py-2 transition hover:border-rose-200 hover:text-rose-600">
                        حذف
                      </button>
                    </div>
                  </td>
        ...
        <div className="p-5">
          <EmptyState
            ...
            description="با ایجاد یا بارگذاری سند، جدول مدیریت اسناد در این بخش فعال می‌شود."
          />
        </div>
      )}
    </Card>
...
function SignaturePanel({ data }: { data: DashboardData }) {
  ...
  const pending = data.signatureRequests.filter(
  ...
  return (
    <Card className="p-6">
      ...
      <div
        className="mx-auto mt-6 grid size-44 place-items-center rounded-full border-[14px] border-slate-100"
        style={{
          background: `conic-gradient(#0F766E ${completion * 3.6}deg, transparent 0)`,
        }}
      >
      ...
      <div className="mt-6 grid gap-3">
        {pending.slice(0, 3).map((request) => (
          ...
          >
            ...
            <p className="mt-1 text-xs text-slate-500">
              {request.signer} · مهلت {request.dueAt}
            </p>
          </div>
        ))}
        {!pending.length ? (
          <EmptyState
            title="امضای معوق وجود ندارد"
            description="همه درخواست‌های امضا تکمیل یا تعیین‌تکلیف شده‌اند."
          />
        ) : null}
      </div>
    </Card>
            ...
            {data.workflows.slice(0, 4).map((workflow) => (
              ...
              >
                ...
                <p className="mt-2 text-xs font-bold text-slate-500">
                  {toFaNumber(workflow.progress)}٪ پیشرفت
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {[
            ...
            ...data.securityEvents.slice(0, 2).map((event, index) => ({
              ...
              description:
                typeof event === "string" ? "رویداد امنیتی" : event.detail,
              createdAt: "",
            })),
          ].map((item) => (
            <div
    ...
    <Card>
      ...
      <div className="space-y-2 p-6">
        {actions.map(([label, icon]) => (
          <button
            className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-right text-sm transition-colors hover:bg-muted/50"
            key={label}
            type="button"
          >
...
function SignaturesContent({ data }: { data: DashboardData }) {
  const groups = [
    [
      "در انتظار امضا",
      data.signatureRequests.filter((item) => item.status === "pending"),
    ],
    [
      "اسناد امضا شده",
      data.signatureRequests.filter((item) => item.status === "signed"),
    ],
    [
      "امضاهای رد شده",
      data.signatureRequests.filter((item) => item.status === "rejected"),
    ],
  ] as const;
  return (
      ...
      <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        ...
        <Card className="overflow-hidden">
          <div className="border-b border-slate-100 p-5">
            <h2 className="text-lg font-black">درخواست‌های امضا</h2>
          </div>
    ...
    <div className="grid gap-6">
        ...
        {data.templates.map((template) => (
          <Card className="p-5" key={template.id}>
            <div className="flex items-start justify-between gap-4">
              <span className="grid size-12 place-items-center rounded-2xl bg-[#ECFDF5] text-[#0F766E]">
                <Icon name="template" />
              </span>
            ...
            <p className="mt-2 min-h-14 text-sm leading-7 text-slate-500">
      ...
      {!data.templates.length ? (
        <EmptyState
          ...
          description="قالب‌های سند پس از ایجاد یا Seed در این بخش دیده می‌شوند."
        />
      ) : null}
    </div>
  );
    ...
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {data.contacts.map((contact) => (
          <Card className="p-5" key={contact.id}>
            <div className="flex items-center gap-3">
              <span className="grid size-12 place-items-center rounded-full bg-slate-100 font-black text-slate-700">
      ...
      {!data.contacts.length ? (
        <EmptyState
          ...
          description="مخاطبین حقیقی و سازمانی پس از ثبت در این بخش نمایش داده می‌شوند."
        />
      ) : null}
    </div>
  );
  ...
  return (
    <div className="grid gap-6">
          ...
          <Card className="p-6" key={workflow.id}>
            ...
            <div className="mt-5 grid gap-3">
              {workflow.steps.map((step, index) => (
                ...
                >
                  <span
                    className={`grid size-8 place-items-center rounded-full text-xs font-black ${step.status === "completed" ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-500"}`}
                  >
                    {toFaNumber(index + 1)}
                  </span>
      ...
      {!data.workflows.length ? (
        <EmptyState
          title="گردش کاری ثبت نشده است"
          description="Workflowهای پایگاه داده پس از ایجاد در این بخش نمایش داده می‌شوند."
        />
      ) : null}
    </div>
  );
...
function PageHeader({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div>
        <p className="text-xs font-black text-[#0F766E]">LegalTech workspace</p>
        <h2 className="mt-1 text-2xl font-black text-slate-950">{title}</h2>
      </div>
      <Link
        className="inline-flex h-11 items-center gap-2 rounded-2xl bg-[#071326] px-5 text-sm font-black text-white"
        href={href}
      >
        <Icon name="plus" className="size-4" />
        {cta}
...
function SimpleContent({
  ...
  if (page === "archive")
    return (
      <DocumentsTable
        documents={data.archivedDocuments}
        title="بایگانی اسناد"
      />
    );
  if (page === "bulk-send") {
          ...
          <div className="grid gap-4 md:grid-cols-4">
            {[
              ...
              <div className="rounded-2xl bg-slate-50 p-5" key={step}>
                <span className="grid size-10 place-items-center rounded-full bg-[#0F766E] font-black text-white">
                  {toFaNumber(index + 1)}
                </span>
...
export function LegalTechPlatform({
  data,
  page = "dashboard",
}: {
  data: DashboardData;
  ...
  const content =
    page === "documents" ? (
      <DocumentsTable documents={data.documents} title="مرکز اسناد" />
    ) : page === "signatures" ? (
      <SignaturesContent data={data} />
    ) : page === "templates" ? (
      <TemplatesContent data={data} />
    ) : page === "contacts" ? (
      <ContactsContent data={data} />
    ) : page === "workflows" ? (
