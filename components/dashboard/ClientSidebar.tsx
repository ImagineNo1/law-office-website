...
export function ClientSidebar() {
  ...
  return (
    <>
      <div
        className="sticky top-0 z-40 border-b border-sidebar-border bg-sidebar/95 px-4 py-3 text-sidebar-foreground shadow-sm backdrop-blur lg:hidden"
        data-tour="client-sidebar"
      >
        <Link
          className="mb-3 flex items-center gap-3 text-sidebar-foreground"
          href="/dashboard"
        >
          <LegalLogo compact />
        ...
        <nav
          aria-label="ناوبری موبایل پورتال مشتری"
      ...
      <aside
        className="hidden border-l border-sidebar-border bg-sidebar px-4 py-6 text-sidebar-foreground shadow-[-18px_0_55px_rgba(12,27,51,0.08)] lg:sticky lg:top-0 lg:order-2 lg:block lg:min-h-screen lg:w-[306px]"
        data-tour="client-sidebar"
      >
        <Link
        ...
        <nav className="grid gap-2">
          {links.map((link) => {
            ...
            return (
              <Link
                className={`flex items-center justify-between rounded-lg px-4 py-3 text-sm font-extrabold transition ${
                  active
                    ? "bg-emerald-700 text-white shadow-sm"
