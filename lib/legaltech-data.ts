...
export const legalDocuments: LegalDocumentRecord[] = Array.from(
  ...
  (_, index) => {
    ...
    return {
      ...
      tags: [categoryFa[category], requiresSignature ? "نیازمند امضا" : "آرشیوی"],
    };
  },
);

export const contacts: ContactRecord[] = Array.from({ length: 50 }, (_, index) => ({
  id: `contact-${index + 1}`,
  fullName: people[index % people.length],
  email: `client${index + 1}@example.com`,
  phone: `۰۹۱۲${toFaNumber(String(3000000 + index * 731).padStart(7, "0"))}`,
  organization: index % 4 === 0 ? "شرکت توسعه پارس" : index % 3 === 0 ? "وکیل پایه یک" : "موکل حقیقی",
  role: ["موکل", "وکیل", "مدیر شرکت", "کارشناس مالی", "نماینده حقوقی"][index % 5],
  tags: [["قرارداد", "امضا"], ["دعاوی", "فوری"], ["شرکتی"], ["مالی"], ["VIP"]][index % 5],
  notes: "دارای دسترسی کنترل شده به اسناد و گردش کارهای مرتبط.",
  avatar: "",
  group: ["گروه قراردادها", "گروه دعاوی", "گروه شرکتی", "گروه امضای انبوه"][index % 4],
}));
...
export const templates: TemplateRecord[] = Array.from({ length: 25 }, (_, index) => ({
  id: `template-${index + 1}`,
  ...
  status: (["active", "active", "draft", "archived"] as TemplateRecord["status"][])[index % 4],
}));
...
export const workflows: WorkflowRecord[] = Array.from({ length: 20 }, (_, index) => {
  ...
  return {
    ...
    steps: [
      ...
      { title: "بررسی اولیه", status: progress > 30 ? "completed" : "active" },
      { title: "تایید نهایی", status: progress > 55 ? "completed" : "queued" },
      { title: "ارسال برای امضا", status: progress > 75 ? "completed" : "queued" },
      { title: "بایگانی", status: progress === 100 ? "completed" : "queued" },
    ],
    assignedRoles: [["Admin", "Lawyer"], ["Lawyer", "Client"], ["Assistant"], ["Paralegal", "Lawyer"]][index % 4],
    status: (["completed", "active", "active", "paused"] as WorkflowRecord["status"][])[index % 4],
    progress,
  };
});
...
export const signatureRequests: SignatureRequestRecord[] = Array.from(
  ...
  (_, index) => ({
    ...
    status: (["pending", "signed", "pending", "rejected", "signed"] as SignatureRequestRecord["status"][])[index % 5],
...
export const storageStats = {
  ...
  breakdown: categories.slice(0, 6).map((category, index) => ({
    ...
    count: legalDocuments.filter((document) => document.category === category).length,
    color: ["#C9973F", "#2563EB", "#16A34A", "#EF4444", "#7C3AED", "#F59E0B"][index],
  })),
};

export const reportSeries = [
  { month: "فروردین", documents: 34, signatures: 24, downloads: 88, clients: 18 },
  { month: "اردیبهشت", documents: 48, signatures: 32, downloads: 126, clients: 24 },
  { month: "خرداد", documents: 42, signatures: 36, downloads: 118, clients: 27 },
  { month: "تیر", documents: 58, signatures: 45, downloads: 142, clients: 31 },
  { month: "مرداد", documents: 76, signatures: 61, downloads: 194, clients: 43 },
  { month: "شهریور", documents: 66, signatures: 54, downloads: 171, clients: 38 },
];
...
export const securityEvents = [
  { label: "ورود جدید به حساب کاربری", detail: "تهران، ایران - ۱۴۰۳/۰۳/۲۶", status: "warning" },
  { label: "امضای سند با موفقیت انجام شد", detail: "قرارداد اجاره ملک تجاری", status: "success" },
  { label: "فضای ذخیره‌سازی در حال پر شدن است", detail: "۶۳٪ از فضای موجود استفاده شده", status: "warning" },
];

export function getDashboardMetrics() {
  const pendingSignatures = signatureRequests.filter((item) => item.status === "pending").length;
  const signedDocuments = legalDocuments.filter((item) => item.status === "signed").length;
